...
trigger_gryffindor.addEventListener("click", function (event) {
    connectionHouse.send("TriggetHouseNotification", "Gryffindor");
    event.preventDefault();
});

trigger_slytherin.addEventListener("click", function (event) {
    connectionHouse.send("TriggetHouseNotification", "Slytherin");
    event.preventDefault();
});

trigger_hufflepuff.addEventListener("click", function (event) {
    connectionHouse.send("TriggetHouseNotification", "Hufflepuff");
    event.preventDefault();
});

trigger_ravenclaw.addEventListener("click", function (event) {
    connectionHouse.send("TriggetHouseNotification", "Ravenclaw");
    event.preventDefault();
});
...
connectionHouse.on("subscriptionStatus", (strGroupsJoined, houseName, hasSubscribed) => {
    ...
    if (hasSubscribed) {
        ...
        toastr.success(`You have subscribed sucessfully. ${houseName}`);
    }
    else {
        //unsubscribe
        switch (houseName) {
        ...
        toastr.success(`You have unsubscribed sucessfully. ${houseName}`);
    }
});

connectionHouse.on("notifyOthers", (user, houseName, hasSubscribed) => {
    if (hasSubscribed) {
        toastr.success(`${user} joined house ${houseName}`);
    }
    else {
        toastr.warning(`${user} left house ${houseName}`);
    }
});

connectionHouse.on("triggetHouseNotification", (houseName) => {
    toastr.success(`Hello from ${houseName}!`);
});
